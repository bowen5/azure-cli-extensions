...
from ._validators import (validate_env, validate_cosmos_type, validate_resource_id, validate_location,
                          ...
                          validate_log_lines, validate_log_limit, validate_log_since, validate_vnet, validate_vnet_required_parameters, validate_node_resource_group)
...
def load_arguments(self, _):
    ...
    with self.argument_context('spring-cloud create') as c:
        ...
        c.argument('reserved_cidr_range', help='Comma-separated list of IP address ranges in CIDR format. The IP ranges are reserved to host underlying Azure Spring Cloud infrastructure, which should be 3 unused /16 IP ranges(or as large as available).', validator=validate_vnet_required_parameters)
